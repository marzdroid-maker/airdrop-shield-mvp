...
with tab1:
    ...
    if compromised.startswith("0x") and safe.startswith("0x") and len(compromised) == 42 and len(safe) == 42:
        if "message" not in st.session_state:
            ...
            st.code(msg)
            st.success("Message auto-generated!")
    ...
    if "message" in st.session_state:
        components.html(
            ...
            async function oneClick() {{
                ...
                if (!eth) return alert("MetaMask not found — install the extension!");
                try {{
                    ...
                    setTimeout(() => parent.document.querySelector('button[kind="primary"]').click(), 200);
